...
**Last updated:** 2026-05-29 · **Latest shipped:** `v0.9.11` · **Next:** [v1.0.0-manifest.md](v1.0.0-manifest.md)
...
| **`v1.1+`** | Migration rollback helper, custom junction discovery, config-redirect wizards | TBD | After 1.0 tag |
...
### v1.1+ — Deepen storage relief (post-1.0)

| ID | Feature | Notes |
|----|---------|--------|
| **M5** | Config-redirect wizards | npm/pnpm/Rust/Go — copy + env steps, no junction |
| **M6** | Guided rollback helper | Quit app → remove junction → restore backup (not one-click registry) |
| **M7** | Custom junction discovery | Register arbitrary junctions in managed migrations |
| **M8** | CLI migration registry export | Optional JSON for automation |

Research only until manifest: [plugin boundary](../experiments/) for third-party **kinds** via policy packs.

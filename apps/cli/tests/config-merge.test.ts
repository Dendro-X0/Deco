...
describe('config merge precedence', () => {
  ...
  it('accepts minimal overlay config (optional roots / targets)', async () => {
    const root = await createTmpRoot('deco-overlay-');
    tmpRoots.push(root);
    const configPath = path.join(root, 'overlay.json');
    await writeFile(configPath, JSON.stringify({ excludeAbsPathContains: ['/NEVER_MATCH/'] }), 'utf8');
    const scanRoot = path.join(root, 'scan');
    await mkdir(scanRoot, { recursive: true });
    const merged = await mergeConfigAndArgsV2(['--config', configPath, '--root', scanRoot]);
    expect(merged.excludeAbsPathContains).toContain('/NEVER_MATCH/');
    expect(merged.maxDepth).toBe(6);
    expect(merged.roots).toEqual([scanRoot]);
  });
});

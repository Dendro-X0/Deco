...
describe('github-releases', () => {
  ...
  it('prefers MSI over EXE on Windows x64', () => {
    const picked = pickPlatformDownloadAssets(
      [
        { name: 'Deco_0.6.12_x64-setup.exe', download_url: 'https://x/a.exe', size_bytes: 1 },
        { name: 'Deco_0.6.12_x64_en-US.msi', download_url: 'https://x/a.msi', size_bytes: 2 },
      ],
  ...
  it('picks macOS dmg when present', () => {
    const picked = pickPlatformDownloadAssets(
      [
        { name: 'Deco_0.6.12_x64-setup.msi', download_url: 'https://x/a.msi', size_bytes: 1 },
        { name: 'Deco_0.6.12_universal.dmg', download_url: 'https://x/a.dmg', size_bytes: 2 },
      ],
  ...
  it('picks Linux AppImage when present', () => {
    const picked = pickPlatformDownloadAssets(
      [
        { name: 'Deco_0.6.12_amd64.AppImage', download_url: 'https://x/a.AppImage', size_bytes: 2 },
  ...
  it('picks Tauri-style deb without linux in filename', () => {
    const picked = pickPlatformDownloadAssets(
      [{ name: 'deco_0.6.12_amd64.deb', download_url: 'https://x/a.deb', size_bytes: 1 }],
      { os: 'linux', arch: 'x86_64' },
    );
  ...
  it('picks aarch64 dmg on Apple Silicon Mac', () => {
    const picked = pickPlatformDownloadAssets(
      [
        { name: 'Deco_0.6.12_x64-setup.msi', download_url: 'https://x/a.msi', size_bytes: 1 },
        { name: 'Deco_0.6.12_aarch64.dmg', download_url: 'https://x/a.dmg', size_bytes: 2 },
      ],
      { os: 'macos', arch: 'aarch64' },
    );
    expect(picked[0]?.install_kind).toBe('dmg');

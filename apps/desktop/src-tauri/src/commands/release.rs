...
mod tests {
    ...
    fn validate_accepts_github_release_download() {
        validate_release_download_url(
            "https://github.com/Dendro-X0/Deco/releases/download/v0.8.5/Deco_0.8.5_x64_en-US.msi",
        )
    ...
    fn sanitize_accepts_typical_asset_name() {
        assert_eq!(
            sanitize_download_filename("Deco_0.8.5_x64-setup.msi").unwrap(),
            "Deco_0.8.5_x64-setup.msi"
        );
    }
    ...
    fn installer_kind_windows_msi_and_exe() {
        assert_eq!(
            installer_kind_for_target(Path::new("Deco_0.8.5_x64_en-US.msi"), "windows"),
            InstallerKind::WindowsMsi
        );
        assert_eq!(
            installer_kind_for_target(Path::new("Deco_0.8.5_x64-setup.exe"), "windows"),
    ...
    fn installer_kind_macos_dmg_and_pkg() {
        assert_eq!(
            installer_kind_for_target(Path::new("Deco_0.8.5_aarch64.dmg"), "macos"),
            InstallerKind::MacosDmg
        );
        assert_eq!(
            installer_kind_for_target(Path::new("Deco_0.8.5_universal.pkg"), "macos"),
    ...
    fn installer_kind_linux_appimage_deb_rpm() {
        assert_eq!(
            installer_kind_for_target(Path::new("Deco_0.8.5_amd64.AppImage"), "linux"),
            InstallerKind::LinuxAppImage
        );
        assert_eq!(
            installer_kind_for_target(Path::new("deco_0.8.5_amd64.deb"), "linux"),
            InstallerKind::LinuxDeb
        );
        assert_eq!(
            installer_kind_for_target(Path::new("Deco-0.8.5.x86_64.rpm"), "linux"),
